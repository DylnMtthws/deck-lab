(function () {
  ...
  function renderText(group, container) {
    ...
    group.entries.forEach(function (entry) {
      ...
      if (imageUrl) { name.href = imageUrl; name.rel = "noopener"; name.setAttribute("aria-label", entry.name + ". View card image"); name.addEventListener("click", function (event) { if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); openCardImage(entry.name, imageUrl, name); }); }
      ...
      if (imageUrl) { var preview = node("button", "dl-icon-button dl-card-preview", ""); preview.type = "button"; preview.setAttribute("aria-label", "View card image for " + entry.name); preview.setAttribute("data-dl-tip", "View card image"); preview.addEventListener("click", function () { openCardImage(entry.name, imageUrl, preview); }); preview.innerHTML = '<svg viewBox="0 0 20 20" aria-hidden="true"><rect x="4" y="3" width="10" height="14" rx="1.5"/><path d="M7 6h10v11a1 1 0 0 1-1 1H7z"/></svg>'; actions.appendChild(preview); }
  ...
  var cardImageDialog = document.getElementById("card-image-dialog"), cardImageOpener = null;
  ...
  if (cardImageDialog) {
    ...
    cardImageDialog.addEventListener("close", function () { var opener = cardImageOpener; cardImageOpener = null; if (opener && opener.focus) opener.focus(); });
  }
